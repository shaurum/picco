...
Сборка группы, включающей в себя <span class="hover-trigger" data-hover="popup1">основной модуль</span>, должна начинаться с установки [Модуля ввода питания SPPM](SPPM.md). Основной модуль всегда устанавливается вторым, после чего устанавливаются модули расширения.

<div id="popup1" class="hover-popup" data-markdown="**Основные модули**
*Основной модуль -  это центральное устройство, которое содержит процессор, память и интерфейсы для управления подключенными устройствами по заданной программе*

В настоящий момент представлен основной модуль **GMB**.
- [Модуль основной GMB](GMB.md)">
</div>
...
*Оконечный модуль - это специальный модуль расширения, который обеспечивает связь двух групп между собой*
...
## Организация питания в группе {#_4}

Для повышения надежности контроллера, организацию питания рекомендуется реализовывать с поддержкой функций ["горячей" замены модулей](#_1) и [резервирование питания](#_2)

[Модуль ввода питания SPPM](SPPM.md) обеспечивает питание всех остальных модулей контроллера. Он позволяет запитать до **48 Вт** нагрузки. Базово [Модуль ввода питания SPPM](SPPM.md) питает установленные справа от него модули, однако, если установлена перемычка, он питает модули установленные  как справа, так и слева от него. При этом максимальная мощность питания остается неизменной - 48 Вт.

Потребляемая мощность для каждого модуля приведена ниже:
...
1. Модули расширения, расположенные справа от рассматриваемого [Модуля ввода питания SPPM](SPPM.md) до следующего 
2. Модули расширения, расположенные слева от рассматриваемого [Модуля ввода питания SPPM](SPPM.md) до следующего
...
На вход [Модуля ввода питания](SPPM.md) подается напряжение **24 В** от двух резервирующих друг друга источников. Модуль автоматически коммутирует на общую шину источник питания с оптимальными параметрами для работы контроллера, что обеспечивает бесперебойное электропитание системы.

!!! success "Рекомендация"
    Контроллер может работать от одного источника питания **без** резервирования, однако рекомендуем всегда организовывать питание с применением функции резервирования питания.

Для бесперебойной работы ["горячей" замены модулей](#_1) необходимо придерживаться следующего правила: все [Модули ввода питания SPPM](SPPM.md), расположенные в одной группе, должны подключаться к одним и тем же источникам питания. 
...
"Горячая" замена модулей позволяет заменить вышедшие из строя модули (планово заменить модули) без остановки работы всего контроллера. 
...
В случае, если на модулях SPPM, расположенных в группе, уже стоит <span class="hover-trigger" data-hover="popup4">перемычка</span>, [демонтируйте](installation.md#_2) неисправный модуль, и [установите](installation.md#_1) исправный на то же место.

!!! success "Полезно"
    Для замены нерабочего модуля нет необходимости демонтировать рабочие.

В случае, если на модулях SPPM, расположенных в группе, не установлены перемычки, установите их на модули SPPM, [демонтируйте](installation.md#_2) неисправный модуль, и [установите](installation.md#_1) исправный на то же место.
...
Для организации разветвленных соединений в сложных топологиях (дерево, звезда, смешанная) используется [Модуль расширения коммутации SPSE](SPSE.md). Организация линейных соединений осуществляется при помощи <span class="hover-trigger" data-hover="popup2">модулей оконечных</span>, установленных в начале и в конце каждой группы.
...
*Модуль расширения коммутации - это специальный модуль расширения, который обеспечивает разветвление шины передачи данных. К модулю расширения коммутации можно подключать более одной шин передачи данных, в отличии от оконечных модулей, подключение более более одной шины к которым приводит к неправильному функционированию системы*
...
!!! success "Рекомендация"
    При сборке контроллера в "кольцо" верхний порт **Модуля основного GMB** рекомендуем оставлять свободным для подключения к нему высокоскоростных внешних устройств.
...
!!! Example "Пояснение"
     На рисунке приведен пример сборки контроллера со "смешанной" топологией

     Группы 1, 2, 3 и 4 последовательно соединены друг с другом, при этом группа 4 подключена к Модулю основному GMB, образуя тем самым сборку "кольцо". Для этих груп действует "горячая" замена модулей и, в случае выхода из строя одного из модулей контроллер сохранит работоспособность.

     Обратите внимание, что группа 5 в схеме "кольцо" не участвует. К контроллеру она подключена через <span class="hover-trigger" data-hover="popup5">модуль расширения коммутации</span>, который расположен в группе 4

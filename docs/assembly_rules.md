...
Для расширения контроллера последующими группами или подключения по схеме [«Кольцо»](assembly.md#_2), в конце группы устанавливается <span class="hover-trigger" data-hover="popup2">модуль оконечный</span>.

!!! success "Рекомендация"
    Рекомендуем в любом случае ставить модуль оконечный в конце группы для дальнейшего подключения по схеме [«Кольцо»](assembly.md#_2)

## Организация питания в группе

Правильная организация питания в группе позволяет реализовать функции ["горячей"" замены модулей]() и [резервирование питания]() контроллера и обеспечивает бесперебойную работу контроллера.

[Модуль ввода питания SPPM]() обеспечивает питание всех остальных модулей контроллера. Он позволяет запитать до **48 Вт** нагрузки. Базово [Модуль ввода питания SPPM]() питает установленные справа от него модули, однако, если установлена перемычка он питает как установленные справа, так и установленные слева от него модули, но максимальная мощность питания остается неизменной - 48 Вт.
...
Сумма потребляемой мощности всех модулей расширения, стоящих справа от рассматриваемого до следующего [Модуля ввода питания SPPM](), **включая сам модуль ввода питания** не должна превышать 48 Вт. 
...
При организации питания с поддержкой **"горячей" замены модулей** каждый [Модуль ввода питания]() отвечает за питание двух зон модулей в группе:

1. Модули расширения, расположенные справа от рассмативаемого до следующего [Модуля ввода питания SPPM]()
2. Модули расширения, расположенные слева от рассмативаемого до следующего [Модуля ввода питания SPPM]()

Каждый [Модуль ввода питания]() устанавливается из расчета, что он питает обе зоны, то есть потребляемая мощность модулей расширения в этих двух зонах, включая сам [Модуль ввода питания](), не должна превышать 48 Вт.
В расчет потребляемой мощности не включается самый левый модуль в левой зоне.

!!! info "Информация"
    В нормальном режиме функционирования [Модуль ввода питания]() питает только правую зону, однако в момент ["горячей" замены]() модулей, он также начинает питать и зону слева от него
...
При сборке группы с функцией ["горячей" замены]() модулей рекомендуем устанавливать <span class="hover-trigger" data-hover="popup4">перемычку</span> на [Модулях ввода питания SPPM]() сразу.
...
Для бесперебойной работы ["горячей" замены модулей]() необходимо придерживаться следующего правила: на каждый [Модуль ввода питания SPPM](), расположенных в одной группе, подается питание от одних и тех же источников питания. 
...
- подключение контроллера по топологии ["Кольцо"]()
- организация питания с поддержкой "горячей" замены модулей
- питание всех [модулей SPPM]() в группе от одних и тех же блоков питания
...
- поддержка функции ["горячей"](power_organization.md#"Горячая"-замена-модулей) замены модулей, что позволяет заменять модули, не прерывая работу контроллера.
